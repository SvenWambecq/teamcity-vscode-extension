...
export class P4PathFinder implements Finder {
    ...
    public async find(): Promise<string> {
        ...
        try {
            return await this.findP4Path(pathHint);
        } catch (err) {
            throw new Error("p4 command line util not found");
        }
    }

    private async findP4Path(pathHint: string): Promise<string> {
        ...
        return firstSearchPromise.then(void 0, () => this.checkPath("p4"));
    }

    private getPathHint(): string {
        return this.workspaceProxy.getConfigurationValue(Constants.P4_LOCATION_SETTING_NAME);
    }
